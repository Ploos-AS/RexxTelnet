...
static void on_text(void *opaque, unsigned char byte)
{
    ...
    if (capture->len < sizeof(capture->data))
        capture->data[capture->len++] = byte;
}

static void on_control(void *opaque,
...
static void test_decoded_text_reaches_terminal(void)
{
    ...
    require(rt_app_session_pump(&app, on_text, on_control, &capture) == 2,
            "pump text");
...
static void test_do_naws_sends_will_and_dimensions(void)
{
    ...
    const unsigned char expected[] = {
        RT_IAC, RT_WILL, RT_TELNET_OPT_NAWS,
        RT_IAC, RT_SB, RT_TELNET_OPT_NAWS,
        0u, 80u, 0u, 24u,
        RT_IAC, RT_SE
    };
    ...
    require(rt_app_session_pump(&app, on_text, on_control, &capture) == 3,
            "pump NAWS");
    require(app.session.local_naws == 1u, "NAWS state");
    require(fake_tx_len == sizeof(expected), "NAWS tx length");
    require(memcmp(fake_tx, expected, sizeof(expected)) == 0,
            "NAWS reply and payload");
}
...
static void test_keyboard_iac_is_escaped(void)
{
    ...
    require(rt_app_session_connect(&app, "example", 23u) == 0,
            "connect keyboard");
    require(rt_app_session_send_input(&app, input, sizeof(input)) == 4,
            "send keyboard");
    require(fake_tx_len == sizeof(expected), "keyboard tx length");
    require(memcmp(fake_tx, expected, sizeof(expected)) == 0,
            "keyboard IAC escape");
}
